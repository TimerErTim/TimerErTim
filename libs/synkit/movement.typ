...
#let _blank-box() = box(width: 2em, height: 0.8em, stroke: (
  bottom: 0.5pt + black,
))

#let _t-trace-box(subscript, fsz, sub-fsz) = {
  (
    text(size: fsz, style: "italic", "t")
      + text(
        size: sub-fsz,
        baseline: 0.2em,
        style: "italic",
        _apply-symbols(subscript),
      )
  )
}
...
#let _move-format-word(raw, fsz, sub-fsz) = {
  ...
  let is-bold = (
    main.starts-with("**") and main.ends-with("**") and main.len() >= 5
  )
  let is-italic = (
    not is-bold
  ...
  let is-smallcaps = (
    main.starts-with("@") and main.ends-with("@") and main.len() >= 3
  )
  let is-underline = (
    main.starts-with("&") and main.ends-with("&") and main.len() >= 3
  )
  ...
  // Strip markers
  let inner = if is-bold { main.slice(2, main.len() - 2) } else if is-italic {
    main.slice(1, main.len() - 1)
  } else if is-smallcaps {
    main.slice(1, main.len() - 1)
  } else if is-underline { main.slice(1, main.len() - 1) } else if is-strike {
    main.slice(1, main.len() - 1)
  } else { main }
  ...
  // Apply formatting
  let body = if is-bold {
    text(size: fsz, weight: "bold", display)
  } else if is-italic {
    text(size: fsz, style: "italic", display)
  } else if is-smallcaps {
    text(size: fsz, smallcaps(display))
  } else if is-underline {
    text(size: fsz, underline(display))
  } else if is-strike { text(size: fsz, strike(display)) } else {
    text(size: fsz, _apply-symbols(main))
  }
...
#let _move-assign-anchors(tokens) = {
  ...
  for tok in tokens {
    if tok.type == "word" {
      let key = _strip-anchor(lower(tok.text))
        .replace("'", "bar")
        .replace("'", "bar")
        .replace(" ", "-")
  ...
  for tok in tokens {
    if tok.type == "bracket-open" {
      ...
      if (
        prev-type == "word"
          or prev-type == "trace"
          or prev-type == "bracket-close"
          or prev-type == "blank"
          or prev-type == "t-trace"
      ) {
        x = x + space-w
      }
      let entry-x = x
      ...
      if tok.label != "" {
        sub-w = (
          measure(text(size: sub-fsz, _apply-symbols(tok.label))).width / unit
        )
        x = x + sub-w
      }
      entries.push((
  ...
  context {
    ...
    let canvas-body = cetz.canvas(length: unit, {
      ...
      for arrow in arrows {
        ...
        let paint = if is-dict { arrow.at("color", default: _norm) } else {
          _norm
        }
        let dash-style = if is-dict {
          arrow.at("dash", default: "solid")
        } else { "solid" }
        ...
        if raw-from in name-to-pos and raw-to in name-to-pos {
          ...
          if (rect-count - 1) in delinks {
            let mid-x = (fx + tx) / 2
            _draw-delink(mid-x, bar-y, 1.0, 0.0, (
              paint: paint,
              thickness: a-sw,
            ))
          }
