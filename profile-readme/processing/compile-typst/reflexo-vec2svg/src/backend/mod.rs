...
#[comemo::memoize]
fn render_image_item(img: &ir::ImageItem, enable_inlined_svg: bool) -> SvgText {
    if enable_inlined_svg && img.image.format.starts_with("svg") {
        return SvgText::Plain(inline_svg(&img.image, img.size));
    }
...
// todo: error handling
pub fn inline_svg(image: &ir::Image, size: Size) -> String {
    let svg_content = String::from_utf8_lossy(&image.data);
